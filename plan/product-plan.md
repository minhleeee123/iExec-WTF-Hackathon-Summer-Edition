...
- Điều kiện hoặc thay đổi người dùng yêu cầu: Tối ưu cho Web Client (React + Vite + Ethers), chạy thật end-to-end trên Ethereum Sepolia không dùng mock data.
...
> **NoxSwap** giúp **DeFi Traders & Institutions** thực hiện swap mà **amount, balance và pool reserves không xuất hiện dưới dạng plaintext on-chain**, bằng cách tích hợp iExec Nox và ERC-7984 trực tiếp trên Ethereum Sepolia.
...
- [x] Giao diện Swap DApp responsive, thiết kế operational neobrutalism.
- [x] Kết nối ví Ethereum Sepolia thật (Ethers v6 `BrowserProvider`).
- [x] Hợp đồng thông minh ERC-7984 & NoxSwap triển khai trên Sepolia.
- [x] Luồng mã hóa tham số swap (`@iexec-nox/handle`) và gọi Nox encrypted compute primitives.
...
- Dữ liệu nhạy cảm: Số tiền swap thực tế và số dư cá nhân (được biểu diễn on-chain bằng Nox handles).
- Biện pháp bảo vệ tối thiểu: Không lưu private key trong source/server; plaintext input được gửi qua TLS tới Nox Gateway theo security model của Handle SDK và không gửi tới public RPC/mempool.
...
|---|---|---|
| Thời gian thực hiện Swap | Hoàn thành trong 120 giây trên Sepolia | Quay screen recording video demo |
...
- Bước 4: NoxSwap thực hiện AMM math qua Nox primitives -> Cập nhật reserve và balance handles mã hóa.
...
- App information architecture: Bốn workspace cấp cao `Trade`, `Wallet`, `Safe Treasury`, `Activity`; personal custody và Safe custody không dùng chung một tab. Safe dùng compact custody header cho identity, module state, balance, reveal và funding, sau đó là bốn section URL-addressable: Swap & Unwrap, Orders & Agent, Activity, Access & Security.
...
|---|---|---|
| JUD-001 (Creativity ⭐⭐⭐) | Confidential Liquidity & Swap Router qua Nox | Demo trong video & Slide giải thích giảm amount leakage; không tuyên bố miễn nhiễm mọi MEV |

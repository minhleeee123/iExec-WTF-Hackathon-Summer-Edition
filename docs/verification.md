...
| Read-only deployment consistency | `npm run verify:deployment` compares canonical artifacts with Sepolia deployment transactions, constructor arguments, receipts, deployed runtime code, and Sourcify exact-source evidence | PASS from a clean checkout for Router V2, Safe Module V6, Safe confidential orderbook, and per-account Safe factory. A clean Hardhat build merges compilation jobs and changes metadata embedded with `NoxSafeModule` creation code inside the factory; the verifier therefore accepts the factory only when Sourcify reports exact creation/runtime matches and its compiler version, executable settings, target identity, resolved source paths, and source contents all match the current build input. It performs no verification submission. |

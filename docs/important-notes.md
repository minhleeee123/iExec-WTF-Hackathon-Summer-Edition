...
- Optimized frontend deployment `dpl_84NAvALNnaQqBFsAoZeCtzK4CLS9` is READY and aliased to `https://noxswap-iexec.vercel.app`; `/`, `/docs`, `/app/wallet`, `/app/safe`, `robots.txt`, and `sitemap.xml` returned HTTP 200, and a production browser smoke confirmed the enabled V5 owner context and canonical module address.

...
const getMarketContext = async () => {
  const feed = new Contract(deployment.feeds.ethUsd, CHAINLINK_FEED_ABI, provider);
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (name === 'nox_confidential_swap') {
      ...
      const inputContract = new Contract(input.address, CONFIDENTIAL_TOKEN_ABI, wallet);
      ...
      const router = new Contract(addresses.noxSwapRouter, NOX_SWAP_ABI, wallet);
    ...
    if (name === 'nox_decrypt_balance') {
      ...
      const contract = new Contract(token.address, CONFIDENTIAL_TOKEN_ABI, provider);
    ...
    if (name === 'nox_get_pool_handles') {
      const router = new Contract(addresses.noxSwapRouter, NOX_SWAP_ABI, provider);
    ...
    if (name === 'nox_create_limit_order') {
      ...
      const orderBook = new Contract(addresses.limitOrderBook, LIMIT_ORDER_ABI, wallet);
    ...
    if (name === 'nox_get_limit_order') {
      const orderBook = new Contract(addresses.limitOrderBook, LIMIT_ORDER_ABI, provider);

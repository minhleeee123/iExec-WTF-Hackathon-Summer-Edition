...
import { createLiveEvidence, writeEvidence } from '@noxswap/contracts/evidence';
...
try {
  ...
  if (writeEnabled) {
    ...
    const evidencePath = process.env.MCP_EVIDENCE_FILE
      ?? path.join(repositoryRoot, 'packages/contracts/artifacts/evidence/mcp-write-e2e.json');

...
contract NoxSwapOptimized is ERC721, Ownable {
    ...
    euint256 private _feeNumeratorHandle;
    euint256 private _feeDenominatorHandle;
    euint256 private _zeroHandle;
    ...
    constructor() ERC721("NoxSwap Confidential Receipt", "NOX-R") Ownable(msg.sender) {
        _feeNumeratorHandle = Nox.toEuint256(FEE_NUMERATOR);
        _feeDenominatorHandle = Nox.toEuint256(FEE_DENOMINATOR);
        _zeroHandle = Nox.toEuint256(0);
        Nox.allowThis(_feeNumeratorHandle);
        Nox.allowThis(_feeDenominatorHandle);
        Nox.allowThis(_zeroHandle);
    }

    function getConstantHandles()
        external
        view
        returns (bytes32 feeNumerator, bytes32 feeDenominator, bytes32 zero)
    {
        return (
            euint256.unwrap(_feeNumeratorHandle),
            euint256.unwrap(_feeDenominatorHandle),
            euint256.unwrap(_zeroHandle)
        );
    }
    ...
    function _settleSwap(
        ...
        euint256 feeAdjusted = Nox.div(
            Nox.mul(receivedAmountIn, _feeNumeratorHandle),
            _feeDenominatorHandle
        );
        ...
        euint256 selectedOutput = Nox.select(meetsMinimum, quotedAmountOut, _zeroHandle);
        euint256 selectedRefund = Nox.select(meetsMinimum, _zeroHandle, receivedAmountIn);
